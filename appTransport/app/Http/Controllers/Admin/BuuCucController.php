...
class BuuCucController extends Controller
{
    ...
    public function index()
    {
        $data['buucucs'] = DB::table('buucucs')
            ->paginate(12);
        return view('backend.content.buucuc.index',$data);
    }
    public function add()
    {
        $data['address'] = DB::table('address')->get();
        return view('backend.content.buucuc.add',$data);
    }
    public function store(Request $request)
    {
        ...
        $item->save();
        return redirect()->route('admin.buucuc.index');
    }
    public function edit($id)
    {
        $data['address'] = DB::table('address')->get();
        $data['buucuc'] = BuuCucModel::find($id);
        return view('backend.content.buucuc.edit',$data);
    }
