...
class DonHangController extends Controller
{
    ...
    public function index()
    {
        $data['donhangs']=DB::table('donhangs')
            ...
            ->orderBy('id_nv','ASC')
            ->paginate(24);
        return view('ketoan.content.donhang.index',$data);
    }
    public function add()
    {
        ...
        $data['diemguihangs']=DB::table('diemguihangs')
            ->get();
        return view('ketoan.content.donhang.add',$data);
    }
    ...
    public function store(Request $request)
    {
        ...
        $info_vanchuyen['giacuoc'] = $input['giacuoc'];
        ...
        $khachhang = KhachHangModel::find($input['id_nguoigui']);
        $donhang->id_nv =$khachhang->id_nhanvien;
        $donhang->id_taixe =0;
        $donhang->date_tpnhan =$date['mday'].'-'.$date['mon'].'-'.$date['year'];
        $donhang->save();
        return redirect()->route('ketoan.donhang.index')->with('success-message', 'Tạo đơn hàng thành công!');
    }
    public function update($id,Request $request)
    {
        ...
        $info_nguoinhan = json_decode($donhang->info_nguoinhan);
        $info_nguoinhan->{'ten'} = $input['ten_nguoinhan'];
        $info_nguoinhan->{'sdt'} = $input['phone_nguoinhan'];
        $info_nguoinhan->{'diachi'} = $input['address'];
        $donhang->info_nguoinhan=json_encode( $info_nguoinhan);
        ...
        $donhang->save();
        return redirect()->route('ketoan.donhang.index')->with('success-message', 'Cập nhật đơn hàng thành công!');
    }
    public function edit($madonhang)
    {
        ...
        $data['diemguihangs']=DB::table('diemguihangs')
            ->get();
        return view('ketoan.content.donhang.edit',$data);
    }
    public function destroy($id)
    {
        $item = DonHangModel::find($id);
        $item->delete();
        return redirect()->route('ketoan.donhang.index')->with('success-message', 'Xóa đơn hàng thành công!');
    }
    public function editChiTiet($id,Request $request)
    {
        ...
        $item->save();
        return redirect()->route('ketoan.donhang.index')->with('success-message', 'Sửa đơn hàng thành công!');
    }
    ...
    public function tracking($id,Request $request)
    {
        ...
        return redirect()->route('ketoan.donhang.index')->with('success-message', 'Tracking đơn hàng thành công!');
    }
    public function DeleteTracking($madonhang,$stt)
    {
        ...
        return redirect()->route('ketoan.donhang.index')->with('success-message', 'Xóa Tracking đơn hàng thành công!');
    }
}
