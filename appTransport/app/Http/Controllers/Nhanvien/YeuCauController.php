...
class YeuCauController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('user.content.yeucau.index',$data);
    }
    public function add()
    {
        ...
        $data['nvkhos'] = DB::table('nvkhos')
            ->get();
        return view('user.content.yeucau.add',$data);
    }
    public function store(Request $request)
    {
        ...
        $yeucau->save();
        return redirect()->route('user.yeucau.index')->with('success-message', 'đã thêm yêu cầu mới!');
    }
    ...
    public function destroy($id)
    {
        ...
        $item->delete();
        return redirect()->route('user.yeucau.index')->with('success-message', 'đã xóa yêu cầu !');
    }
    public function tiepnhan($id)
    {
        ...
        $item->trangthai=1;
        $item->save();
        return redirect()->route('user.yeucau.index')->with('success-message', 'đã tiếp nhận yêu cầu !');
    }
}
