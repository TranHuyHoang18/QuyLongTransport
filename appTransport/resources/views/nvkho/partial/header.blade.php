        ...
        <div class="row topk">
            <ul>
                @if(Auth::user()==NULL)
                    ...
                    @if (Route::has('nvkho.auth.login'))
                        <a href="{{url('nhan-vien-kho/login') }}"><li style="color:white">Đăng nhập</li></a>
                    @endif
                @else
                    <li><i class="fa fa-user-o userhead" aria-hidden="true" style="font-size: 30px;color: white;float: left"></i></li>
                    <a href="{{ url('nhan-vien-kho/logout') }}"><li style="color:white">Đăng Xuất</li></a>
                @endauth
            </ul>
        </div>
    </div>
